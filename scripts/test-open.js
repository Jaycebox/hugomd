...
(async () => {
  ...
  {
    const userData = path.join(os.tmpdir(), 'hugomd-status-A');
  ...
  {
    const userData = path.join(os.tmpdir(), 'hugomd-status-B');
    if (fs.existsSync(userData)) fs.rmSync(userData, { recursive: true, force: true });
    const broken = path.join(userData, 'workspaces', 'broken');
    fs.mkdirSync(broken, { recursive: true });
    // 干净工作区标识（.hugomd.json）存在，但 posts 里有非法 front matter
    fs.writeFileSync(path.join(broken, '.hugomd.json'), JSON.stringify({ name: 'broken', theme: 'minimal' }, null, 2));
